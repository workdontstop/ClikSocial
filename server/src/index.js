...
WHERE posts.mode = 0 AND posts.sender = ? AND posts.id < ?  
...
app.post("/profileExplainUser", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { values } = req.body;
    if (values.postPageLimit == 0) {
        try {
            const chronologicaldata = yield execPoolQuery(profileExplainUser, [
                values.id,
                values.id2,
                values.id3,
            ]);
        ...
        try {
            const chronologicaldata = yield execPoolQuery(profile_moreExplainUser, [
                values.id,
                values.id2,
                values.id3,
